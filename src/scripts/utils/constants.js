export const avatarInput = document.querySelector("#avatarUpdate");
export const nameInput = document.querySelector("#userName");
export const jobInput = document.querySelector("#about-self");
export const btnOpenPopupAvatar = document.querySelector(
  ".profile__avatar-button"
);
export const btnOpenPopupEdit = document.querySelector(
  ".profile-info__edit-button"
);
export const btnOpenPopupAdd = document.querySelector(".profile__add-button");
export const avatarImg = document.querySelector(".profile__avatar");
export const config = {
  inputSelector: ".form__input-area",
  submitButtonSelector: ".form__button",
  formSelector: ".form",
  inactiveButtonClass: "form__button_disabled",
  inputErrorClass: "form__input-area_error",
  errorClass: ".popup__error",
};

export const selectors = {
  popupSelector: ".popup",
  popupFormSelector: ".popup_form ",
  cardListSelector: ".elements",
  avatarSelector: ".profile__avatar",
  userNameSelector: ".profile-info__userName",
  aboutSelfSelector: ".profile-info__about-self",
};
